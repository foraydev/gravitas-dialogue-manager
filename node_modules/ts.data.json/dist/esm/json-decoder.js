...
(function (JsonDecoder) {
    var Decoder = /** @class */ (function () {
         ...
         */
        Decoder.prototype.fold = function (onOk, onErr, json) {
         ...
         * @param json The JSON object to decode
         */
        Decoder.prototype.decodeToPromise = function (json) {
            var _this = this;
         ...
         * @param fn The transformation function
         */
        Decoder.prototype.mapError = function (fn) {
            var _this = this;
            return new Decoder(function (json) {
                var result = _this.decodeFn(json);
                if (result.isOk()) {
         ...
         */
        Decoder.prototype.chain = function (fn) {
            var _this = this;
            return new Decoder(function (json) {
                var result = _this.decodeFn(json);
                if (result.isOk()) {
    ...
    JsonDecoder.string = new Decoder(function (json) {
        if (typeof json === 'string') {
            ...
            return err($JsonDecoderErrors.primitiveError(json, 'string'));
        }
    });
    /**
     * Decoder for `number`.
    ...
    JsonDecoder.number = new Decoder(function (json) {
        if (typeof json === 'number') {
            ...
            return err($JsonDecoderErrors.primitiveError(json, 'number'));
        }
    });
    /**
     * Decoder for `boolean`.
    ...
    JsonDecoder.boolean = new Decoder(function (json) {
        if (typeof json === 'boolean') {
            ...
            return err($JsonDecoderErrors.primitiveError(json, 'boolean'));
        }
    });
    /**
     * Decoder for an empty object `{}`.
     */
    JsonDecoder.emptyObject = new Decoder(function (json) {
        if (json !== null &&
            typeof json === 'object' &&
     ...
     */
    function enumeration(enumObj, decoderName) {
        return new Decoder(function (json) {
            var enumValue = Object.values(enumObj).find(function (x) { return x === json; });
            if (enumValue !== undefined) {
                return ok(enumValue);
            }
            return err($JsonDecoderErrors.enumValueError(decoderName, json));
        });
    }
    JsonDecoder.enumeration = enumeration;
    /**
     * Decoder for objects.
     *
    ...
    function object(decoders, decoderName, keyMap) {
        return new Decoder(function (json) {
            if (json !== null && typeof json === 'object') {
                    ...
                    if (decoders.hasOwnProperty(key)) {
                        if (keyMap && key in keyMap) {
                            ...
                            if (r.isOk()) {
                                ...
                                return err($JsonDecoderErrors.objectJsonKeyError(decoderName, key, jsonKey, r.error));
                            }
                        }
                        else {
                            ...
                            if (r.isOk()) {
                                ...
                                return err($JsonDecoderErrors.objectError(decoderName, key, r.error));
                            }
                        }
                    }
                }
                ...
                return err($JsonDecoderErrors.primitiveError(json, decoderName));
            }
        });
    }
    ...
    function objectStrict(decoders, decoderName) {
        return new Decoder(function (json) {
            if (json !== null && typeof json === 'object') {
                for (var key in json) {
                    if (!decoders.hasOwnProperty(key)) {
                        return err($JsonDecoderErrors.objectStrictUnknownKeyError(decoderName, key));
                    }
                }
                var result = {};
                for (var key in decoders) {
    ...
    function oneOf(decoders, decoderName) {
        return new Decoder(function (json) {
            ...
            return err($JsonDecoderErrors.oneOfError(decoderName, json));
        });
    }
    ...
    JsonDecoder.dictionary = function (decoder, decoderName) {
        return new Decoder(function (json) {
            if (json !== null && typeof json === 'object') {
                ...
                for (var key in json) {
                    if (json.hasOwnProperty(key)) {
                        ...
                        if (result.isOk()) {
                            ...
                            return err($JsonDecoderErrors.dictionaryError(decoderName, key, result.error));
                        }
                    }
                }
                return ok(obj);
                ...
                return err($JsonDecoderErrors.primitiveError(json, 'dictionary'));
            }
        });
    };
    /**
            ...
            if (json instanceof Array) {
                ...
                for (var i = 0; i < json.length; i++) {
                    ...
                    if (result.isOk()) {
                        ...
                        return err($JsonDecoderErrors.arrayError(decoderName, i, result.error));
                    }
                }
                return ok(arr);
            }
            else {
                return err($JsonDecoderErrors.primitiveError(json, 'array'));
            }
        });
    };
    JsonDecoder.tuple = function (decoders, decoderName) {
        return new Decoder(function (json) {
            if (json instanceof Array) {
                var arr = [];
                if (json.length !== decoders.length) {
                    return err($JsonDecoderErrors.tupleLengthMismatchError(decoderName, json, decoders));
                }
                for (var i = 0; i < json.length; i++) {
                    var result = decoders[i].decode(json[i]);
                    if (result.isOk()) {
                        arr.push(result.value);
                    }
                    else {
    ...
    function isNull(defaultValue) {
        return new Decoder(function (json) {
            if (json === null) {
                ...
                return err($JsonDecoderErrors.nullError(json));
            }
        });
    }
    ...
    function isUndefined(defaultValue) {
        return new Decoder(function (json) {
            if (json === undefined) {
                ...
                return err($JsonDecoderErrors.undefinedError(json));
            }
        });
    }
    ...
    JsonDecoder.constant = function (value) {
        return new Decoder(function (json) { return ok(value); });
    };
    /**
    ...
    function isExactly(value) {
        return new Decoder(function (json) {
            if (json === value) {
                ...
                return err($JsonDecoderErrors.exactlyError(json, value));
            }
        });
    }
...
(function ($JsonDecoderErrors) {
    ...
    $JsonDecoderErrors.enumValueError = function (decoderName, invalidValue) {
        return "<" + decoderName + "> decoder failed at value \"" + invalidValue + "\" which is not in the enum";
    };
    ...
    $JsonDecoderErrors.tupleLengthMismatchError = function (decoderName, jsonArray, decoders) {
        return "<" + decoderName + "> tuple decoder failed because it received a tuple of length " +
            (jsonArray.length + ", but " + decoders.length + " decoders.");
    };
})($JsonDecoderErrors || ($JsonDecoderErrors = {}));
